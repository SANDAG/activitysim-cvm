...
class Random(object):
    ...
    def get_channel_for_df(self, df):
        ...
        logger.debug(
            f"available channel index names include {list(self.index_to_channel.keys())}"
        )
    ...
    def add_channel(self, channel_name, domain_df):
        ...
        if channel_name in self.channels:
            ...
            logger.debug(
                "Adding channel '%s' %s ids index=%s"
                % (channel_name, len(domain_df.index), domain_df.index.name)
            )
