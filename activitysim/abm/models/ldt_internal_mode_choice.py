...
def ldt_internal_mode_choice(
    ...
    assert not (ldt_tours.tour_category == "atwork").any()

    logger.info("Running %s with %d tours" % (trace_label, ldt_tours.shape[0]))

    tracing.print_summary("tour_types", ldt_tours.tour_type, value_counts=True)
    ...
    # don't create estimation data bundle if mode choice is being called
    # from another model step
    ...
    for tour_purpose, tours_segment in ldt_tours_merged.groupby(segment_column_name):
        ...
        tracing.print_summary(
            "ldt_internal_mode_choice %s choices_df" % tour_purpose,
    ...
    tracing.print_summary(
        "ldt_internal_mode_choice all tour type choices",
        choices_df.tour_mode,
        value_counts=True,
    )
