...
from activitysim.core import inject, pipeline
from .ldt_pattern import LDT_PATTERN

logger = logging.getLogger(__name__)

@inject.step()
def create_longdist_trips(longdist_tours, persons_merged):
    """
    ...
    res = longdist_tours.to_frame().copy().reset_index().drop(["tour_type_count", "tour_type_num", "tour_num", "tour_count"], axis=1)
    ...
    res["destination"] = np.where(res["internal_destination"] != -1, res["internal_destination"], res["external_destchoice"])
    res["mode_choice"] = np.where(res["external_tour_mode"] != -1, res["external_tour_mode"], res["tour_mode"])
    ...
    res["origin"] = np.where((res["ldt_pattern"] & 3) == LDT_PATTERN.END, res["destination"], res["origin"])
    res["destination"] = np.where((res["ldt_pattern"] & 3) == LDT_PATTERN.END, temp, res["destination"])
    ...
    household_to_persons = persons_merged.to_frame()[["household_id"]].reset_index().set_index("household_id")
    new_rows = []
    for _, row in households.iterrows():
        id = row["household_id"]
        if type(household_to_persons.loc[id, "person_id"]) == np.int64:
    ...
    res["mode_choice"] = np.where(res["actor_type"] == "household", res["mode_choice"] + res["number_of_participants"].astype(str), res["mode_choice"])
    
    # convert longdist_tours csv into a longdist_trips csv
    pipeline.replace_table("longdist_trips", res)
